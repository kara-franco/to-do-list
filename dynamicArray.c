 ...
 * lallyk@onid.oregonstate.edu		        CS 261-400	    *
 * Due Date: August 3, 2015                                 *
 * Program Description: This program demonstrates a dynamic *
 * array implementation of the heap abstract data type.     *
 ***********************************************************/
...
{
	// asset heap is not empty
	assert(sizeDynArr(heap) > 0);
	// return the first node (at position 0) this has the lowest priority
...
void addHeap(DynArr *heap, TYPE val)
{
	...
	assert(heap != 0);
	// place holder for parent node
	int parent;
	// add node to heap
	addDynArr(heap, val);
	// set current to position of new node
	int current = sizeDynArr(heap) - 1;
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	// assert the max is less than the size of array
	assert(max <= sizeDynArr(heap));
	// set up the positions to make the adjustments
	int left = (2 * pos) + 1;
	int right = (2 * pos) + 2;
	int smallestChild;
	...
	if (right <= max){
		...
		// if the value at smallest is less than the value at position swap
		if (compare(heap->data[smallestChild], heap->data[pos]) == -1){
...
void removeMinHeap(DynArr *heap)
{
	...
	if (last != 0){
		// move the element to the first position (last - 1)
...
void _buildHeap(DynArr *heap)
{
	...
	// iterate through to build the heap
...
void sortHeap(DynArr *heap)
{
	...
	_buildHeap(heap);
	// sort the heap using swap and adjust (compare function used)
	for (int index = sizeDynArr(heap) - 1; index > 0; index--){
		swapDynArr(heap, 0, index);
		_adjustHeap(heap, index - 1, 0);
	}
